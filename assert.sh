...
# Disable history expansion.  Command strings and expected output regularly
# contain '!' (e.g. "#!/bin/bash"); in an interactive shell that '!' would
# otherwise be treated as a history event reference and emit a stray
# "<word>: event not found" error -- pure noise on top of passing tests.
set +H

_assert_parse_opts() {
    # Parse command-line options.  This is wrapped in a function on purpose:
    # assert.sh is normally *sourced*, and the "set --" below must rebind only
    # *this function's* positional parameters.  Done at the top level it would
    # clobber the positional parameters ("$@") of the script that sourced us.
    local args
    args="$(getopt -n "$0" -o vhxdic -l \
        verbose,help,stop,discover,invariant,continue -- "$@")" \
    || exit 1
    eval set -- "$args"
    while [[ $# -gt 0 ]]; do
        case "$1" in
